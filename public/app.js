...
function renderEmployeeTimeline() {
  ...
  // So o ultimo evento do dia pode estar "em andamento", e so quando a
  // jornada ainda esta aberta (trabalhando ou em pausa).
  const journey = computeJourneyState();
  const isJourneyOpen = journey.status === "working" || journey.status === "break";
  ...
  recordsList.innerHTML = `<ol class="punch-timeline">${todayRecords
    .map((record, index) => {
      ...
      const mapLink = mapsUrl
        ...
        : "";
      const isActive = isJourneyOpen && index === lastRecordIndex;
      const activeBadge = isActive ? '<span class="punch-active-badge">Em andamento</span>' : "";

      return `
        <li class="punch-item ${PUNCH_ITEM_CLASSES[record.action] || ""}${record.pending ? " punch-pending" : ""}${isActive ? " punch-active" : ""}">
          <span class="punch-time">${escapeHtml(getShortTime(record))}</span>
          <span class="punch-body">
            <span class="punch-label">${escapeHtml(formatPunchLabel(record.action))}${activeBadge}</span>
