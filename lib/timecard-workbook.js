...
function computeSummary(records) {
  ...
  const activeWorkdayByEmployee = new Map();

  for (const row of [...records].sort(compareRecordsForSummary)) {
    const employeeKey = String(row.employee_id || "");
    const vehiclePlate = row.vehicle_plate || "Nao informado";
    let workday = activeWorkdayByEmployee.get(employeeKey) || row.local_date;

    if (row.action === "Entrada") {
      workday = row.local_date;
      activeWorkdayByEmployee.set(employeeKey, workday);
    }

    const key = `${row.employee_id}::${workday}::${vehiclePlate}`;
    if (!buckets.has(key)) {
      buckets.set(key, {
    ...
    if (row.action === "Saida") {
      ...
      activeWorkdayByEmployee.delete(employeeKey);
    }
  }
  ...
  return [...buckets.values()].map((item) => {
    ...
    let activeLunchAt = null;
    let breaksDuringShiftMs = 0;

    for (const event of item.events) {
      if (event.action === "Entrada") {
        activeEntryAt = event.recordedAt;
        activeLunchAt = null;
        breaksDuringShiftMs = 0;
        continue;
      }

      if (event.action === "Saida para almoco" && activeEntryAt && !activeLunchAt) {
        activeLunchAt = event.recordedAt;
        continue;
      }

      if (event.action === "Retorno do almoco" && activeLunchAt) {
        const lunchMs = new Date(event.recordedAt) - new Date(activeLunchAt);
        intervalMs += lunchMs;
        breaksDuringShiftMs += lunchMs;
        activeLunchAt = null;
        continue;
      }

      if (event.action === "Saida" && activeEntryAt) {
        workedMs += new Date(event.recordedAt) - new Date(activeEntryAt) - breaksDuringShiftMs;
        activeEntryAt = null;
        activeLunchAt = null;
        breaksDuringShiftMs = 0;
      }
