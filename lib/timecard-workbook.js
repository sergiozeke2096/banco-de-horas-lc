...
// Excel guarda horario/duracao como fracao de um dia (24h = 1). Convertendo
// minutos pra esse formato, a celula fica com valor numerico de verdade —
// da pra somar/filtrar no Excel, nao so ler como texto "08:30".
function minutesToExcelTime(totalMinutes) {
  return Math.max(totalMinutes, 0) / (24 * 60);
}
...
// overridesById vem do banco (server.js, buildWorkloadOverridesMap): Map de
// employeeId -> minutos, pra jornadas diferentes de 8h (ex.: motorista de
// 12h). Esse modulo nao acessa banco direto, entao quem chama e responsavel
// por montar e passar esse mapa.
function getDailyWorkloadMinutes(employeeId, overridesById) {
  const normalizedEmployeeId = String(employeeId || "").trim();
  const override = overridesById?.get(normalizedEmployeeId);

  return typeof override === "number" && !Number.isNaN(override) ? override : DEFAULT_DAILY_WORKLOAD_MINUTES;
}
...
function computeSummary(records, vehicleTransfers = [], workloadOverridesById) {
  ...
  return [...buckets.values()].map((item) => {
    ...
    const overtimeMinutes = Math.max(totalMinutes - getDailyWorkloadMinutes(item.employeeId, workloadOverridesById), 0);
...
function createWorkbook(records, vehicleTransfers = [], workloadOverridesById) {
  ...
  const summaryRows = computeSummary(sortedRecords, vehicleTransfers, workloadOverridesById).sort((left, right) => {
  ...
  // Resumo vem primeiro (aba mais facil de ler); Registros (dados crus) e
  // Resumo Detalhado (muitas colunas) ficam depois pra quem precisar delas.
  ...
  const summaryBaseColumns = [
    ...
    { header: "Intervalo", key: "interval", width: 14, style: horasNumFmt },
    { header: "Horas trabalhadas", key: "worked", width: 18, style: horasNumFmt },
    { header: "Horas extras", key: "overtime", width: 16, style: horasNumFmt },
  ...
  for (const row of summaryRows) {
    const baseSummaryRow = {
      ...
      interval: minutesToExcelTime(row.intervalMinutes),
      worked: minutesToExcelTime(row.workedMinutes),
      overtime: minutesToExcelTime(row.overtimeMinutes),
      dailyKm: row.dailyKm,
    };
  ...
  [
    ...
    ["Intervalo", "Tempo total de intervalo registrado entre saida e retorno do almoco (valor de hora real, pode ser somado no Excel)."],
    ["Horas trabalhadas", "Total diario calculado descontando o intervalo de almoco (valor de hora real, pode ser somado no Excel)."],
    ["Horas extras", "Tempo trabalhado acima da carga horaria diaria configurada para o funcionario (valor de hora real, pode ser somado no Excel)."],
